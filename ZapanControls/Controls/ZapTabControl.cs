...
namespace ZapanControls.Controls
{
    ...
    public sealed class ZapTabControl : TabControl
    {
        ...
        {
            LastSelectedTab = (e?.RemovedItems?.Count ?? 0) > 0 ? e.RemovedItems[0] : this.Items?[(this.Items?.Count ?? 0) > 0 ? this.Items.Count - 1 : 0];

            if ((e?.AddedItems?.Count ?? 0) > 0)
            {
                if (e.AddedItems[0] is ZapTabItemAdd)
                    this.SelectedItem = LastSelectedTab;
            }
        ...
        {
            this.SetBinding(Control.BorderBrushProperty, new Binding("BorderBrush") { Source = this });

            this.SelectionChanged += InternalZapTabControl_SelectionChanged;
        ...
        protected override void OnInitialized(EventArgs e)
        {
            if (CanAddItem)
            {
                this.Items.Insert(0, new ZapTabItemAdd() { ToolTip = AddButtonTooltip, Header = AddButtonHeader });
